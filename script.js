...
const FAITHFLOW_CONTENT = {
  'community': {
    ...
    thankMessage: 'We received your message about FaithFlow community. Below is the heart of what FaithFlow is, how Iron and Ember is modeling it, and a few resources you can use while we review your note.',
    sections: [
      {
        type: 'prose',
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Knowing God by J.I. Packer', desc: 'A foundational book on the character and nature of God.', url: 'https://www.crossway.org/books/knowing-god-tpb/' }
        ]
      }
    ]
  },
  ...
  'future-group': {
    ...
    thankTitle: 'Thank You for Letting Us Know.',
    thankMessage: 'We received your message about being considered for a future group. While we review it, here are a few simple ways to build a strong foundation in faith before any group forms.',
    sections: [
      {
        type: 'prose',
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Mere Christianity by C.S. Lewis', desc: 'A short, clear book that explains the heart of Christian faith honestly.', url: 'https://www.harpercollins.com/products/mere-christianity-c-s-lewis' },
  ...
  'help-start': {
    ...
    thankTitle: 'Thank You for Being Willing.',
    thankMessage: 'We received your message about helping start a group. That kind of step should be handled carefully, so below is what FaithFlow looks for in future leaders and what we ask leaders to carry.',
    sections: [
      {
        type: 'prose',
      ...
      },
      {
        ...
        items: [
          ...
          { ref: '1 Peter 5:2-3', text: 'Be shepherds of God\'s flock that is under your care... not lording it over those entrusted to you, but being examples to the flock.' },
      ...
      },
      {
        ...
        items: [
          { name: 'The Trellis and the Vine by Marshall and Payne', desc: 'On building people, not programs. The clearest book on disciple-making we know of.', url: 'https://matthiasmedia.com/products/the-trellis-and-the-vine' },
          { name: 'Spiritual Leadership by J. Oswald Sanders', desc: 'A timeless guide on what humble Christian leadership actually looks like.', url: 'https://www.moodypublishers.com/books/christian-living/spiritual-leadership/' },
  ...
  'learn-more': {
    ...
    thankTitle: 'Thank You for Wanting to Learn.',
    thankMessage: 'We received your message. Here is the simple version of FaithFlow in one place so you do not have to search through the whole page to understand the heart of it.',
    sections: [
      {
        type: 'prose',
        heading: 'What FaithFlow Is',
      ...
      },
      {
        ...
        items: [
          ...
          { name: 'Mere Christianity by C.S. Lewis', desc: 'A timeless explanation of the heart of Christian faith. Short, clear, and honest.', url: 'https://www.harpercollins.com/products/mere-christianity-c-s-lewis' },
          { name: 'The Reason for God by Tim Keller', desc: 'Christianity for thoughtful skeptics. Addresses the hardest questions head-on.', url: 'https://www.penguinrandomhouse.com/books/178708/the-reason-for-god-by-timothy-keller/' },
...
const FAITHFLOW_STORAGE_KEY = 'faithflow-interest';

function normalizeFaithflowInterest(value) {
  return Object.prototype.hasOwnProperty.call(FAITHFLOW_CONTENT, value)
    ? value
    : 'learn-more';
}

function readStoredFaithflowInterest() {
  try {
    return window.sessionStorage.getItem(FAITHFLOW_STORAGE_KEY);
  } catch (err) {
    return null;
  }
}

function storeFaithflowInterest(value) {
  try {
    window.sessionStorage.setItem(FAITHFLOW_STORAGE_KEY, normalizeFaithflowInterest(value));
  } catch (err) {
    // Storage may be blocked in private browsing. The form action still carries the value.
  }
}
...
function populateFaithflowResources(interestValue) {
  ...
  const successTitleEl = document.querySelector('#ffSuccess .ff-success-title');
  const successMsgEl   = document.querySelector('#ffSuccess .ff-success-msg');
  ...
  if (successTitleEl) successTitleEl.textContent = content.thankTitle || 'Thank You.';
  if (successMsgEl) successMsgEl.textContent = content.thankMessage || "Your message has been received. We'll review it and reach out when there is a fitting next step.";
...
function showFormSuccess(form, preferredInterest) {
  ...
  if (formName === 'faithflow') {
    ...
    const interest    = getFaithflowInterest(form, preferredInterest);

    storeFaithflowInterest(interest);
...
// We also read the interest value from the URL or session storage so the right content shows up.
(function checkSubmittedParam() {
  const params = new URLSearchParams(window.location.search);
  if (params.get('submitted') === '1') {
    const ffForm = document.getElementById('faithflowForm');
    if (ffForm) {
      const interest = getFaithflowInterest(ffForm, params.get('interest'));
      const select = ffForm.querySelector('[name="interest"]');
      if (select) {
        select.value = interest;
      }
      showFormSuccess(ffForm, interest);
